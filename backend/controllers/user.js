...
router.post(
  ...
  async (req, res, next) => {
    ...
    if (userExists) {
      ...
      if (!userExists.isActive && userExists) {
        await deleteFile(fileUrl).catch((err) => {
          console.log("Error deleting file:", err.message);
        });
        return next(
          new ErrorHandler("Go to your email to activate your account", 400)
        );
      }
...
router.post(
  ...
  catchAsyncError(async (req, res, next) => {
    ...
    try {
      ...
      if (user.isActive) {
        return next(new ErrorHandler("Account already activated Login", 400));
      }
